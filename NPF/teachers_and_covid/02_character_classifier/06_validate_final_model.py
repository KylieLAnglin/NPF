...
all_tweets = pd.read_csv(
    start.CLEAN_DIR + "tweets_relevant_labeled.csv", index_col="unique_id"
)
all_tweets = all_tweets.rename(columns={"label": "classification_label"})

df = all_tweets.merge(
    validation_df[["human_label"]], how="right", left_index=True, right_index=True
)
len(df)
# %%
print(accuracy_score(df.human_label, df.classification_label))
